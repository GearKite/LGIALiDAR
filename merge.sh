...
src_dir="./output/buildings/"

out_dir="./output/buildings_merged/"
...
merge_directory() {
    ...
    local merged_file="$out_dir$dir_name.tif"
    
    # Check if the merged file already exists
    if [ ! -f "$merged_file" ]; then
        echo Merging $dir_name
        gdalwarp "$dir"/*.tif "$merged_file" -co COMPRESS=ZSTD -co BIGTIFF=YES -co TILED=YES
    else
